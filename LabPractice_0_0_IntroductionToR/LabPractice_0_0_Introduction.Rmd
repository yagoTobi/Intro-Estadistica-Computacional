...
Collaborated with: Yago Tobio, Luis Bueno, Victor Ovejero, Jesús López
...
```{r q8d}
# YOUR CODE GOES HERE - Observar en el documento word
```
...
mean_svi <- colMeans(pros.dat.svi)
mean_non_svi <- colMeans(pros.dat.no.svi)

var_svi <- pros.dat.svi.sd^2
var_non_svi <- pros.dat.no.svi.sd^2

# Compute the denominators of the t-statistics
pros.dat.denom <- sqrt(var_svi / n + var_non_svi / m)

# Compute the t-statistics
pros.dat.t.stat <- (mean_svi - mean_non_svi) / pros.dat.denom
...
```{r q14b}
sx2 <- pros.dat.svi.sd^2  # Variance for SVI group
sy2 <- pros.dat.no.svi.sd^2  # Variance for non-SVI group
n <- nrow(pros.dat.svi)
m <- nrow(pros.dat.no.svi)

pros.dat.df <- ((sx2 / n + sy2 / m)^2) / ((sx2 / n)^2 / (n - 1) + (sy2 / m)^2 / (m - 1))
...
print(pros.dat.p.val)

# Identify variables with p-value < 0.05
significant_vars <- which(pros.dat.p.val < 0.05)

# Print the indices of significant variables
cat("Variables with significant differences:", significant_vars, "\n")

# Identify the variable with the smallest p-value
most_significant_var <- which.min(pros.dat.p.val)

# Print the index of the most significant variable
cat("Variable with the most significant difference:", most_significant_var)
...
repeat {
  var_name <- readline(prompt="Enter a variable name or 'quit' to exit: ")
  
  if (var_name == "quit") {
    break
  }
  
  # Ensure pros.dat is a dataframe
  if (!is.data.frame(pros.dat)) {
    pros.dat <- as.data.frame(pros.dat)
  }
  
  if (var_name %in% colnames(pros.dat)) {
    # Ensure the column is numeric
    numeric_column <- as.numeric(pros.dat[, var_name])

    if (!all(is.na(numeric_column))) {
      hist(numeric_column, main=paste("Histogram of", var_name), xlab=var_name)
    } else {
      print(paste("The variable", var_name, "cannot be converted to numeric or contains non-numeric data."))
    }
  } else {
...
repeat {
  ...
  plot_type <- readline(prompt="Enter the plot type ('histogram', 'scatterplot', or 'quit' to exit): ")
  ...
  if (plot_type == "histogram") {
    var_name <- readline(prompt="Enter a variable name for the histogram: ")
    ...
    var_name_x <- readline(prompt="Enter the X variable name for the scatterplot: ")
    var_name_y <- readline(prompt="Enter the Y variable name for the scatterplot: ")
    
    if (var_name_x %in% colnames(pros.dat) && var_name_y %in% colnames(pros.dat)) {
      numeric_column_x <- as.numeric(pros.dat[, var_name_x])
      numeric_column_y <- as.numeric(pros.dat[, var_name_y])
      
      if (!all(is.na(numeric_column_x)) && !all(is.na(numeric_column_y))) {
        plot(numeric_column_x, numeric_column_y, main=paste("Scatterplot of", var_name_x, "vs.", var_name_y), xlab=var_name_x, ylab=var_name_y)
      } else {
        print("One or both specified variables cannot be converted to numeric or contain non-numeric data.")
      }
    } else {
      print("Oops! One or both specified variables are not in my data set.")
    }
  } else {
    print("Unsupported plot type. Please enter 'histogram' or 'scatterplot'.")
  }
}
```
...
**16c.** Add two columns to `state.df`, containing the x and y coordinates (longitude and latitude, respectively) of the center of the states, that are stored in the (existing) list `state.center`. Hint: take a look at this list in the console, to see what its elements are named. Name these two columns `Center.x` and `Center.y`. Display the first 3 rows and all 12 columns of `state.df`.
...
**16d.** Make a new data frame which contains only those states whose longitude is less than -100. Do this in two different ways: using manual indexing, and `subset`(). Check that they are equal to each other, using an appropriate function call.
...
**16e.** Make a new data frame which contains only the states whose longitude is less than -100, and whose murder rate is above 9%. Print this new data frame to the console. Among the states in this new data frame, which has the highest average life expectancy?
...
huber(1) # Should return 1
huber(4) # Should return 7
```
...
print(huber(3, 2))  # Should return 8
print(huber(3, 4))  # Should return 9
```
...
```{r q22d}
# Check with named arguments
print(huber(a=1, x=3))

# Check with positional arguments
print(huber(1, 3)) # first argument is x and the second one is a
```
...
# Test the vectorized function with the provided example
...
x.vals <- 1:6
a <- 3
huber.vals <- sapply(x.vals, function(x) huber(x, a))

# Plot the Huber function values against the x values
plot(x.vals, huber.vals, type = 'b', col = 'blue', xlab = 'x values', ylab = 'Huber values', main = 'Huber Function Plot')

# Add a grid for easier visualization, to know 'a' just solve the equation  when x > a
grid()
```
...
```{r q23a}
# Assuming the huber function is defined as follows
huber <- function(x, a = 1) {
...
# Calculate Huber values for these x values
huber.vals <- sapply(x.vals, huber)

# Plot the Huber function
plot(x.vals, huber.vals, type = 'l', lwd = 2, xlab = "x", ylab = "Huber value", main = "Huber Function")
abline(v = c(-1, 1), col = "red", lty = 2) # Add red dotted lines at x = -1 and x = 1
text(x = c(-3, 0, 3), y = c(10, 0.5, 10), labels = c("Linear", "Quadratic", "Linear"), col = "red") # Add text
```
...
# Call the modified function to demonstrate the side effect
huber_val <- huber(2) 
print(huber_val)       # print the function's return value to see it
```
...
huber <- function(x, a = 1) {
  ...
  plot(1, type="n", xlab="", ylab="", xlim=c(0, 1), ylim=c(0, 1), xaxt='n', yaxt='n', asp=1)
  rect(0, 0, 1, 1, col="red") # Background
  rect(0.35, 0.45, 0.65, 0.55, col="white") # Horizontal part of the cross
  rect(0.45, 0.35, 0.55, 0.65, col="white") # Vertical part of the cross
  
  ifelse(abs(x) <= a, x^2, 2 * a * abs(x) - a^2)
}

# Call the modified function to demonstrate the side effects
huber_val <- huber(2)
print(huber_val)
```
...
a <- 1.5
print(huber.sloppy(x=3))  # 6.75

a <- 2
print(huber.sloppy(x=3))  # 8

a <- 0.5
print(huber.sloppy(x=3))  # 2.75

# Challenge: Set a to a string and call huber.sloppy
a <- "test"
print(huber.sloppy(x=3)) #9, resorts to the default value were a is not an int, so a = 0, 9
```
...
huber(x=3, b <- 20) # * 9, as we've defined a to have a default value of 1. 
print(b) # * Gives us 20, even though there is no b present in the function, that's pretty cool. 
```
